...
function engineName(id){return id==='omnivoice'?'OmniVoice HQ':'Qwen3-TTS'}
function voicesFor(engine){return state.voices.voices.filter(v=>(v.engines||['qwen']).includes(engine))}
function fillVoiceSelect(select,engine,selected){const voices=voicesFor(engine);select.innerHTML=voices.map(v=>`<option value="${v.id}">${escapeHtml(v.name)}</option>`).join('');select.value=voices.some(v=>v.id===selected)?selected:(voices.find(v=>v.id===state.voices.default)?.id||voices[0]?.id||'')}
function renderVoiceChoices(){fillVoiceSelect($('#newVoice'),$('#newEngine')?.value||'qwen',$('#newVoice')?.value||state.voices.default);if(state.current){const engine=state.current.tts_engine||'qwen';$('#projectEngine').value=engine;fillVoiceSelect($('#projectVoice'),engine,state.current.voice_id||state.voices.default)}}
function renderRuntime(){
  const icons={ffmpeg:'▶',ffprobe:'⌁',yt_dlp:'↓',asr:'≋',tts:'◉'};
  const names={ffmpeg:'FFmpeg',ffprobe:'FFprobe',yt_dlp:'yt-dlp',asr:'Faster-Whisper',tts:'Qwen + OmniVoice'};
...
function renderProjects(){
  ...
  $('#kpiProjects').textContent=state.projects.length;$('#kpiMinutes').textContent=`${Math.round(state.projects.reduce((n,p)=>n+Number(p.duration||0),0)/60)} min`;$('#kpiReady').textContent=state.projects.filter(p=>p.status==='done').length;$('#kpiEngine').textContent=`${Object.values(state.runtime.tts?.engines||{}).filter(x=>x.ready).length} prêts`;
...
function renderStudio(){
  ...
  const engine=p.tts_engine||'qwen',engineRuntime=state.runtime.tts?.engines?.[engine]||{};$('#statSegments').textContent=p.segments?.length||'—';$('#statDuration').textContent=p.duration?fmt(p.duration):'—';$('#statWarnings').textContent=p.segments?.filter(s=>!['ideal','tolerable'].includes(s.sync?.status)).length||0;$('#statTtsLabel').textContent=`Estimation ${engineName(engine)} / 1 h`;$('#statTtsEstimate').textContent=p.tts_benchmark?.engine_id===engine?`≈ ${p.tts_benchmark.estimated_minutes_for_one_hour} min`:`≈ ${engineRuntime.estimated_minutes_per_hour||31.2} min`;
...
async function startTask(kind,payload={}){await api(`/api/projects/${state.current.id}/tasks/${kind}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});monitorTasks();toast(`${kind==='tts'?`Génération ${engineName(state.current.tts_engine||'qwen')}`:{analyze:'Analyse',export:'Export'}[kind]} lancée en arrière-plan`)}
function nextStep(p){
  ...
  if(p.status==='script_ready')return{title:'Générer les voix',copy:`${engineName(p.tts_engine||'qwen')} traite tout le lot avec progression et ETA.`,action:'Tout générer',run:generateAll};
...
$('#projectEngine').onchange=async e=>{const engine=e.target.value;fillVoiceSelect($('#projectVoice'),engine,state.current.voice_id);try{state.current=await api(`/api/projects/${state.current.id}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({tts_engine:engine,voice_id:$('#projectVoice').value})});renderStudio();toast(`${engineName(engine)} sélectionné`)}catch(err){toast(err.message,true);renderStudio()}};
$('#projectVoice').onchange=async e=>{try{state.current=await api(`/api/projects/${state.current.id}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({tts_engine:$('#projectEngine').value,voice_id:e.target.value})});toast(`Voix ${engineName(state.current.tts_engine||'qwen')} sélectionnée`)}catch(err){toast(err.message,true)}};
