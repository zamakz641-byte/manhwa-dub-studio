...
def load_fast_model(model_path: str):
    ...
    # The studio synthesizes short narration segments. A 1024-token static
    # cache leaves ample headroom while reducing CUDA graph warm-up. Keep BF16
    # on supported GPUs: FP16 was numerically unstable with this local model.
    dtype = torch.bfloat16 if torch.cuda.is_bf16_supported() else torch.float16
    legacy.MODEL = FasterQwen3TTS.from_pretrained(
        str(path), device="cuda:0", dtype=dtype, attn_implementation="sdpa", max_seq_len=1024,
