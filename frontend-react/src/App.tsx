...
export default function App() {
  ...
  useEffect(() => {
    if (currentProject?.id) startTaskPolling(currentProject.id);
    return () => {
      if (taskTimerRef.current) clearInterval(taskTimerRef.current);
    };
  }, [currentProject?.id, startTaskPolling]);
  ...
  // Create project
  const handleCreateProject = async (title: string, language: string, voiceId: string, engine: TtsEngine) => {
    try {
  ...
  const handleUploadSourceFile = async (file: File) => {
    ...
    try {
      ...
      const updated = await uploadSourceFile(currentProject.id, file, setUploadProgress);
  ...
  const handleGenerateAllTts = async () => {
    ...
    try {
      ...
      const engineName = currentProject.tts_engine === 'supertonic' ? 'Supertonic 3 CUDA' : currentProject.tts_engine === 'omnivoice' ? 'OmniVoice HQ' : 'Qwen3-TTS';
      showToast(`Génération du lot ${engineName} lancée`);
      startTaskPolling(currentProject.id);
    } catch (err: any) {
  ...
  // Change voice
  const handleChangeVoice = async (voiceId: string, engine?: TtsEngine) => {
    if (!currentProject) return;
    try {
  ...
  const handleChangeEngine = async (engine: TtsEngine) => {
