...
interface NewProjectModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (title: string, language: string, voiceId: string, engine: 'qwen' | 'omnivoice') => Promise<void>;
...
export const NewProjectModal: React.FC<NewProjectModalProps> = ({
  ...
  const [engine, setEngine] = useState<'qwen' | 'omnivoice'>('qwen');
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              ...
              <select
                id="newEngine"
                value={engine}
                onChange={(e) => setEngine(e.target.value as 'qwen' | 'omnivoice')}
                className="w-full px-3 py-2 rounded-lg bg-[#0d0f14] border border-[#1f2128] text-xs text-white focus:outline-none focus:border-[#fbbf24]/80 cursor-pointer"
              >
