...
interface StudioViewProps {
  ...
  onChangeEngine: (engine: TtsEngine) => Promise<void>;
}
...
export const StudioView: React.FC<StudioViewProps> = ({
  ...
  const displayedProgress = primaryTask ? primaryTask.progress : project?.progress || 0;
          ...
          <div className="flex items-center flex-wrap gap-2.5 shrink-0">
            <div className="flex items-center gap-2 px-2.5 py-1 rounded-lg bg-[#0d0f14] border border-[#1f2128] text-xs">
              ...
              <select
                ...
                onChange={(e) => onChangeEngine(e.target.value as TtsEngine)}
              ...
              >
                ...
                <option value="supertonic" className="bg-[#14161c]">Supertonic 3 CUDA</option>
              </select>
            </div>
            <div className="flex items-center gap-2 px-2.5 py-1 rounded-lg bg-[#0d0f14] border border-[#1f2128] text-xs">
              ...
              <div className="text-center">
                ...
                <small className="text-[10px] text-[#63666d] block mt-0.5">
                  {uploadProgress === null
                    ...
                    : uploadProgress < 99
                    ? 'Copie locale de la vidéo — gardez cette fenêtre ouverte'
                    : 'Lecture des métadonnées vidéo…'}
                </small>
              </div>
              {uploadProgress !== null && (
                <div className="w-full max-w-md h-1.5 overflow-hidden rounded-full bg-[#1f2128]">
                  <div
                    className="h-full rounded-full bg-[#fbbf24] transition-[width] duration-150"
        ...
        <div className="grid grid-cols-12 gap-4">
          ...
          <div className="col-span-12 lg:col-span-4 bento-card p-4 flex flex-col justify-between space-y-3">
            ...
            {activeSegForEditor && (
              <div className="flex items-center gap-2 pt-2">
                ...
                >
                  {generatingSegmentId === activeSegForEditor.id ? (selectedEngine === 'supertonic' ? 'Supertonic...' : selectedEngine === 'omnivoice' ? 'OmniVoice...' : 'Qwen...') : 'TTS'}
                </button>
              </div>
            )}
          </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-xs font-bold text-white">
                      {selectedVoiceObj?.name || 'Narrateur'} ({selectedEngine === 'supertonic' ? 'Supertonic 3' : selectedEngine === 'omnivoice' ? 'OmniVoice HQ' : 'Qwen3-TTS'})
                    </div>
              ...
              <div className="flex items-center justify-between p-2.5 bg-[#0d0f14] border border-[#1f2128] rounded-lg opacity-70 hover:opacity-100 transition-opacity">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded bg-[#1f2128] flex items-center justify-center text-white font-bold text-sm">
                    {selectedEngine === 'supertonic' ? 'S' : selectedEngine === 'omnivoice' ? 'O' : 'Q'}
                  </div>
                  <div>
                    <div className="text-xs font-bold text-white">{selectedEngine === 'supertonic' ? 'Supertonic 3 CUDA' : selectedEngine === 'omnivoice' ? 'OmniVoice HQ Local' : 'Qwen Local Engine'}</div>
                    <div className="text-[10px] text-[#63666d]">{selectedEngine === 'supertonic' ? 'M1/M5 · 16 steps · vitesse 1,2 · GPU' : selectedEngine === 'omnivoice' ? 'Clone vocal français · worker persistant' : 'Génération de masse accélérée'}</div>
                  </div>
                </div>
          ...
          >
            <div className="text-center">
              <div className="text-black font-black text-sm uppercase tracking-tighter">
                Générer toute la narration ({selectedEngine === 'supertonic' ? 'Supertonic' : selectedEngine === 'omnivoice' ? 'OmniVoice' : 'Qwen'})
              </div>
              <div className="text-black/70 text-[10px] uppercase font-bold tracking-widest mt-0.5">
                {project.segments?.length || 0} segments · {selectedEngine === 'supertonic' ? '≈ 2,4 min / heure sur RTX 4060' : selectedEngine === 'omnivoice' ? '≈ 8 min / heure sur Naturelle FR' : '≈ 31 min / heure mesurées'}
              </div>
            </div>
          </div>
            ...
            <div className="px-4 py-3 bg-[#1a1c22] border-b border-[#1f2128] flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div className="flex gap-3 text-[10px] font-mono">
                  ...
                  {warningCount > 0 && (
                    <span className="text-yellow-400 font-bold">↔ {warningCount} segment(s) fortement retimé(s)</span>
                  )}
                </div>
              </div>
            ...
            <div id="segmentList" className="divide-y divide-[#1f2128]">
              {visibleSegments.length === 0 ? (
                ...
                visibleSegments.map((seg) => {
                  ...
                  const isOverTarget = wordCount > wordBudget;
                      ...
                      <div className="grid grid-cols-12 gap-3 items-center">
                        ...
                        <div className="col-span-12 md:col-span-5 space-y-1">
                          ...
                          <div className="flex items-center justify-between text-[10px] font-mono">
                            <span className={isOverTarget ? 'text-yellow-400 font-bold' : 'text-green-400'}>
                              {wordCount}/{wordBudget} mots cible {isOverTarget && '· vidéo retimée'}
                            </span>
          ...
          <div className="col-span-12 lg:col-span-6 bento-card p-4 space-y-3">
            ...
            <div className="flex items-center justify-between">
              <small id="taskSummary" className="text-[10px] font-mono text-[#63666d]">
                {primaryTask
                  ? `${primaryTask.message} · ${primaryTask.current}/${primaryTask.total} · ${formatEta(primaryTask.eta_seconds)}`
          ...
          <div className="col-span-12 lg:col-span-6 bento-card p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              ...
              <div className="p-2.5 rounded bg-[#0d0f14] border border-[#1f2128]">
                <span id="statTtsLabel" className="text-[10px] text-[#63666d] block uppercase">Temps {selectedEngine === 'supertonic' ? 'Supertonic' : selectedEngine === 'omnivoice' ? 'OmniVoice' : 'Qwen'} / 1h</span>
                <b id="statTtsEstimate" className="font-mono text-base text-[#fbbf24] font-bold">
                  ≈ {project.tts_benchmark?.engine_id === selectedEngine
                    ...
                    : selectedEngine === 'supertonic' ? 2.4 : selectedEngine === 'omnivoice' ? 8 : 31.2} min
                </b>
              </div>
            </div>
          </div>
