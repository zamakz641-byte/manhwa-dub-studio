...
SUPERTONIC_PYTHON = env_path(
    "MANHWA_SUPERTONIC_PYTHON",
    str(DUBROOM / "data/environments/tts-supertonic-3/venv/Scripts/python.exe"),
)
SUPERTONIC_MODEL_ROOT = env_path(
    "MANHWA_SUPERTONIC_MODEL_ROOT",
    str(DUBROOM / "models/supertonic-3"),
)
SUPERTONIC_WORKER = ROOT / "supertonic_worker.py"
SUPERTONIC_CUDA_LIBS = env_path(
    "MANHWA_SUPERTONIC_CUDA_LIBS",
    str(DUBROOM / "data/environments/tts-omnivoice-hq-torch28-v53/venv/Lib/site-packages/torch/lib"),
)

def qwen_ready() -> bool:
    return QWEN_PYTHON.is_file() and QWEN_MODEL.exists() and QWEN_WORKER.is_file()
...
def supertonic_ready() -> bool:
    return (
        SUPERTONIC_PYTHON.is_file()
        and (SUPERTONIC_MODEL_ROOT / "onnx" / "vocoder.onnx").is_file()
        and (SUPERTONIC_MODEL_ROOT / "voice_styles" / "M1.json").is_file()
        and (SUPERTONIC_MODEL_ROOT / "voice_styles" / "M5.json").is_file()
        and SUPERTONIC_WORKER.is_file()
        and SUPERTONIC_CUDA_LIBS.is_dir()
    )
        ...
        try:
            ...
            if voices:
                ...
                for voice in voices:
                    ...
                    if qwen_available and Path(str(voice.get("prompt_path") or "")).is_file():
                        engines.append("qwen")
                    voice["engines"] = engines
                    voice["omnivoice_prompt_path"] = str(omni_prompt)
                if supertonic_ready():
                    preview_root = ROOT / "projects" / "complet-15b1d5" / "previews" / "supertonic3"
                    voices.extend([
                        {
                            "id": "supertonic_m1", "name": "Supertonic M1 · Narrateur",
                            "language": "fr", "gender": "male", "style": "Narration claire et dynamique",
                            "description": "Voix officielle Supertonic 3 · CUDA · 16 steps · vitesse 1,2",
                            "source": "Supertonic 3 · preset officiel M1", "engines": ["supertonic"],
                            "supertonic_voice": "M1", "reference_path": str(preview_root / "M1-16steps-vitesse-1.2-continu-ameliore.wav"),
                            "tags": ["GPU", "16 steps", "1.2×"], "ready": True,
                        },
                        {
                            "id": "supertonic_m5", "name": "Supertonic M5 · Narrateur",
                            "language": "fr", "gender": "male", "style": "Narration profonde et stylée",
                            "description": "Voix officielle Supertonic 3 · CUDA · 16 steps · vitesse 1,2",
                            "source": "Supertonic 3 · preset officiel M5", "engines": ["supertonic"],
                            "supertonic_voice": "M5", "reference_path": str(preview_root / "preset-M5-speed-1.2-16steps-normalized.wav"),
                            "tags": ["GPU", "16 steps", "1.2×"], "ready": True,
                        },
                    ])
...
def status() -> dict:
    ...
    return {
        ...
        "tts": {
            "ready": qwen_ready() or omnivoice_ready() or supertonic_ready(),
            ...
            "engines": {
                "qwen": {
                    "id": "qwen", "name": "Qwen3-TTS", "ready": qwen_ready(),
                    "estimated_minutes_per_hour": 31.2,
                },
                "omnivoice": {
                ...
                "supertonic": {
                    "id": "supertonic", "name": "Supertonic 3 CUDA", "ready": supertonic_ready(),
                    "estimated_minutes_per_hour": 2.4,
                    "note": "Mesure locale M1/M5, 16 steps, vitesse 1,2 : environ 25,7× temps réel.",
                },
            },
        },
        "omni_voice": {
        ...
        "supertonic": {
            ...
            "engine": "Supertonic 3 · ONNX CUDA",
            "python": str(SUPERTONIC_PYTHON),
            "model": str(SUPERTONIC_MODEL_ROOT),
            "voice_count": 2,
            "measured_audio_per_compute": 25.7,
            "estimated_minutes_per_hour": 2.4,
            "benchmark_note": "M1/M5 officiels, 16 steps, vitesse 1,2, RTX 4060 Laptop",
        },
    }
