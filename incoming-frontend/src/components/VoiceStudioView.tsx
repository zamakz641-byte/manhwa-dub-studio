...
import type { VoicesResponse, Voice, Project } from '../types';

interface VoiceStudioViewProps {
  voicesData: VoicesResponse | null;
  activeProject: Project | null;
  onSelectVoice: (voiceId: string, engine?: 'qwen' | 'omnivoice') => Promise<void>;
}
...
export const VoiceStudioView: React.FC<VoiceStudioViewProps> = ({
  ...
  const [engineFilter, setEngineFilter] = useState<'all' | 'Qwen3-TTS' | 'OmniVoice'>('all');
  ...
  const filteredVoices = voices.filter((v) => {
    ...
    const id = engineFilter === 'OmniVoice' ? 'omnivoice' : 'qwen';
                ...
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded bg-[#1f2128] text-[#fbbf24] border border-[#fbbf24]/20 uppercase tracking-wider">
                      {(voice.engines || ['qwen']).map((engine) => engine === 'omnivoice' ? 'OmniVoice' : 'Qwen').join(' + ')}
                    </span>
              ...
              <div className="pt-3 border-t border-[#1f2128] flex items-center justify-between gap-2">
                ...
                {activeProject && (
                  <button
                    type="button"
                    onClick={() => {
                      const requested = engineFilter === 'OmniVoice' ? 'omnivoice' : engineFilter === 'Qwen3-TTS' ? 'qwen' : undefined;
